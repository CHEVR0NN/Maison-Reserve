...
const TRUCK_TINT = { truck_1: "#3F9C7E", truck_2: "#7C89A6" };
...
const TRUCK_STYLE = [
  { stroke: "#63BC9C", fill: "#3F9C7E", icoStyle: { background: "rgba(63,156,126,0.16)", border: "1px solid var(--verdigris-deep)" }, no: "t1" },
  { stroke: "#7C89A6", fill: "#5A6A85", icoStyle: { background: "rgba(124,137,166,0.16)", border: "1px solid #435066" }, no: "t2" },
];
  ...
  useEffect(() => {
    ...
    import("leaflet").then((mod) => {
      ...
      Object.values(trucks).forEach((t) => {
        ...
        const color = TRUCK_TINT[t.id] || "#3F9C7E";
        ...
        const html = `<div style="display:flex;flex-direction:column;align-items:center;opacity:${live ? 1 : 0.5}">`
          + `<div style="width:18px;height:18px;border-radius:50%;background:${color};border:2px solid #101216;box-shadow:0 0 0 2px ${color};transform:rotate(${t.position.heading || 0}deg)"></div>`
    ...
    import("leaflet").then((mod) => {
      ...
      Object.values(trucks).forEach((t) => {
        ...
        t.stops.forEach((s, i) => {
          ...
          if (!c) return;
          const html = `<div style="width:22px;height:22px;border-radius:50%;background:${color};color:#101216;`
            + `font:700 11px ui-sans-serif,sans-serif;display:flex;align-items:center;justify-content:center;`
            + `border:2px solid #101216;box-shadow:0 0 0 1px ${color}">${i + 1}</div>`;
      ...
      if (depot) {
        const whIcon = L.divIcon({ html: `<div style="width:14px;height:14px;border-radius:3px;background:#EBEDE9;border:2px solid #101216"></div>`, className: "", iconSize: [14, 14], iconAnchor: [7, 7] });
      ...
      <div className="deliv-grid">
        ...
        <div className="card map-card">
          ...
          <svg viewBox="0 0 460 380" style={{ width: "100%", height: "auto" }}>
            ...
            <path d="M30,150 Q20,90 80,80 Q130,70 150,120 Q160,170 120,200 Q60,220 40,190 Z" fill="url(#zg)" stroke="#454C58" strokeWidth="1.5" />
            <text x="80" y="135" fill="#B4B7B1" fontSize="13" fontFamily="Fraunces, Georgia, serif" fontWeight="700">WEST</text>
            <text x="62" y="152" fill="#888C90" fontSize="10" fontFamily="monospace">W1 W2 W3</text>
            <path d="M160,170 Q150,120 200,115 Q250,110 255,160 Q258,205 215,215 Q170,218 160,190 Z" fill="url(#zg)" stroke="#454C58" strokeWidth="1.5" />
            <text x="190" y="165" fill="#B4B7B1" fontSize="12" fontFamily="Fraunces, Georgia, serif" fontWeight="700">CENTRAL</text>
            <text x="182" y="182" fill="#888C90" fontSize="10" fontFamily="monospace">C1 C2 C3</text>
            <path d="M180,60 Q230,30 290,55 Q320,75 300,115 Q270,140 220,120 Q180,105 180,60 Z" fill="url(#zg)" stroke="#454C58" strokeWidth="1.5" />
            <text x="225" y="80" fill="#B4B7B1" fontSize="13" fontFamily="Fraunces, Georgia, serif" fontWeight="700">NORTH</text>
            <text x="210" y="97" fill="#888C90" fontSize="10" fontFamily="monospace">N1 N2 N3</text>
            <path d="M300,75 Q360,55 405,95 Q430,130 400,165 Q360,185 320,160 Q295,130 300,75 Z" fill="url(#zg)" stroke="#454C58" strokeWidth="1.5" />
            <text x="335" y="115" fill="#B4B7B1" fontSize="11" fontFamily="Fraunces, Georgia, serif" fontWeight="700">NE</text>
            <text x="322" y="132" fill="#888C90" fontSize="9" fontFamily="monospace">NE1 NE2 NE3</text>
            <path d="M280,200 Q340,180 400,210 Q435,235 405,275 Q355,300 300,275 Q270,245 280,200 Z" fill="url(#zg)" stroke="#454C58" strokeWidth="1.5" />
            <text x="330" y="245" fill="#B4B7B1" fontSize="13" fontFamily="Fraunces, Georgia, serif" fontWeight="700">EAST</text>
            <text x="312" y="262" fill="#888C90" fontSize="10" fontFamily="monospace">E1 E2 E3</text>
              ...
              return (
                <g key={truck.id}>
                  ...
                  <g fontFamily="monospace" fontSize="9" fontWeight="700">
                    {g.pts.map((p) => (
                      <g key={p.n}>
                        <circle cx={p.x} cy={p.y} r="9" fill={p.delivered ? g.fill : "#454C58"} stroke={p.delivered ? "none" : g.fill} strokeWidth={p.delivered ? 0 : 1.5} />
                        <text x={p.x} y={p.y + 3.5} fill={p.delivered ? "#101216" : g.stroke} textAnchor="middle">{p.n}</text>
                      </g>
                    ))}
                  </g>
                </g>

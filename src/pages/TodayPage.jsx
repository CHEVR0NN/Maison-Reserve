...
export default function TodayPage({ setTab }) {
  ...
  for (const p of criticalItems.slice(0, 2)) {
    attn.push({ bar: "crit", tag: "CRITICAL", tagClass: "crit", t: <><b>{p.sku}</b> {p.name} below reorder point</>, x: `On hand ${p.stock} · reorder at ${p.minStock}` });
  }
  if (lowItems.length > 0) {
    attn.push({ bar: "warn", tag: "LOW STOCK", tagClass: "warn",
  ...
  if (expiringMembers.length > 0) {
    attn.push({ bar: "info", tag: "LOYALTY", tagClass: "info",
  ...
  for (const c of attentionChannels) {
    attn.push({ bar: "warn", tag: "SYNC", tagClass: "warn", t: <><b>{c.label}</b> sync is running behind</>, x: "Check the Marketplace page for channel health" });
  }
  ...
  return (
    <section className="panel active" id="today">
      <div className="panel-head">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 2, background: "var(--surface-2)", border: "1px solid var(--line)", borderRadius: 10, padding: 3 }}>
          {RANGE_TABS.map(({ key, label }) => {
            const active = range === key;
            return (
              <button key={key} onClick={() => setRange(key)} style={{
                padding: "5px 14px", borderRadius: 8,
                border: `1px solid ${active ? "var(--honey-deep)" : "transparent"}`,
                background: active ? "var(--amber-glow)" : "transparent",
                color: active ? "var(--honey-2)" : "var(--muted)",
                fontSize: 12, fontWeight: active ? 700 : 500, cursor: "pointer", lineHeight: 1.4, whiteSpace: "nowrap",
              }}>
                {label}
              </button>
            );
        ...
        <div>
          <div className="card feeds-card">
            ...
            {(() => {
              ...
              const feeds = [
                { id: "own-site", name: "Own Site", meta: <>Live checkout feed &middot; <b>{todaysByChan.Web || 0} orders</b> ingested today</>, icon: (<svg viewBox="0 0 24 24" fill="none" stroke="var(--honey)" strokeWidth="2"><circle cx="12" cy="12" r="9" /><path d="M3 12h18M12 3a14 14 0 010 18M12 3a14 14 0 000 18" /></svg>), state: chanState(todaysByChan.Web || 0), sub: "Streaming" },
                { id: "lazada",   name: "Lazada",   meta: <><b>{todaysByChan.Lazada || 0} orders</b> pulled via marketplace sync today</>, icon: (<svg viewBox="0 0 24 24" fill="none" stroke="var(--orange)" strokeWidth="2"><rect x="3" y="5" width="18" height="14" rx="2" /><path d="M3 7l9 6 9-6" /></svg>), state: chanState(todaysByChan.Lazada || 0), sub: "Channel sync" },
                { id: "shopee",   name: "Shopee",   meta: <><b>{todaysByChan.Shopee || 0} orders</b> pulled via marketplace sync &middot; one stock pool</>, icon: (<svg viewBox="0 0 24 24" fill="none" stroke="var(--honey)" strokeWidth="2"><path d="M4 8h16l-1.5 11a2 2 0 01-2 1.8H7.5a2 2 0 01-2-1.8z" /><path d="M9 8a3 3 0 016 0" /></svg>), state: chanState(todaysByChan.Shopee || 0), sub: "Channel sync" },
                { id: "ledger",   name: "Inventory Ledger", meta: <><b>{criticalItems.length + lowItems.length} SKUs</b> flagged across all storefronts &middot; one pool stays accurate</>, icon: (<svg viewBox="0 0 24 24" fill="none" stroke="var(--positive)" strokeWidth="2"><path d="M3 7l9-4 9 4v10l-9 4-9-4z" /><path d="M3 7l9 4 9-4" /></svg>), state: <span className="statepill live"><span className="dot g"></span>RECONCILED</span>, sub: "Auto-deducted" },
                { id: "loyalty",  name: "Loyalty / QR Funnel", meta: <><b>{members.length.toLocaleString("en-SG")} members</b> in the Reserve loyalty programme</>, icon: (<svg viewBox="0 0 24 24" fill="none" stroke="var(--honey)" strokeWidth="2"><rect x="4" y="4" width="7" height="7" rx="1" /><rect x="13" y="4" width="7" height="7" rx="1" /><rect x="4" y="13" width="7" height="7" rx="1" /><path d="M13 13h3v3h-3zM17 17h3v3h-3z" /></svg>), state: <span className="statepill live"><span className="dot g"></span>ACTIVE</span>, sub: `${members.length} members` },
              ];
              return feeds.map((f) => (
                <div key={f.id} className="feed-row" onClick={() => setTab(f.id === "loyalty" ? "Loyalty" : f.id === "ledger" ? "Inventory" : "Marketplace")} style={{ cursor: "pointer" }}>
            ...
            return (
              <div className="card sales-card">
                <div className="row">
                  ...
                  <div style={{ textAlign: "right" }}><div className="card-title muted">Orders today</div><div style={{ fontFamily: "var(--serif)", fontSize: 22, marginTop: 8, color: "var(--cream)" }}>{todays.length}</div></div>
                </div>
                {revenueToday > 0 && (
                  <div style={{ display: "flex", height: 8, borderRadius: 4, overflow: "hidden", marginTop: 14, background: "var(--line)" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 12 }}>
                  {CHANS.map((c) => {
                    ...
                    return (
                      <div key={c.key} style={isEmpty ? { opacity: 0.35 } : undefined}>
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                          <span style={{ color: c.color, fontWeight: 600 }}>{c.label}</span>
                          <span style={{ color: "var(--cream-dim)" }}>{isEmpty ? "—" : <>{SGD(rev)} &middot; {cnt} order{cnt !== 1 ? "s" : ""}</>}</span>
                        </div>
                        <div style={{ height: 5, borderRadius: 3, background: "var(--line)" }}><div style={{ height: "100%", borderRadius: 3, width: `${pct}%`, background: c.color }} /></div>
                      </div>
                    );
                  })}
                  {todays.length === 0 && <div style={{ color: "var(--muted)", fontSize: 13 }}>No orders yet today</div>}
                </div>
              </div>
            );
          })()}
          ...
          {range === "tomorrow" && (() => {
            ...
            const rows = [
              { label: `Truck 1${trucks.truck_1 ? ` · ${trucks.truck_1.driverName}` : ""}`, list: t1, color: "var(--honey)" },
              { label: `Truck 2${trucks.truck_2 ? ` · ${trucks.truck_2.driverName}` : ""}`, list: t2, color: "var(--orange)" },
              ...(unassigned.length ? [{ label: "Unassigned", list: unassigned, color: "var(--muted)" }] : []),
            ];
            return (
              <div className="card sales-card">
                <div className="row">
                  ...
                  <div style={{ textAlign: "right" }}><div className="card-title muted">Queue value</div><div style={{ fontFamily: "var(--serif)", fontSize: 22, marginTop: 8, color: "var(--cream)" }}>{SGD(pendingRevenue)}</div></div>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 16 }}>
                  {rows.map((r) => (
                    <div key={r.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", borderRadius: 8, background: "var(--surface-3)", border: "1px solid var(--line)" }}>
                      <div><div style={{ fontSize: 12, fontWeight: 600, color: r.color }}>{r.label}</div><div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>{r.list.length} stop{r.list.length !== 1 ? "s" : ""}</div></div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "var(--cream-dim)" }}>{SGD(truckRev(r.list))}</div>
                    </div>
                  ))}
                  {pending.length === 0 && <div style={{ color: "var(--muted)", fontSize: 13 }}>Queue is clear — all orders delivered</div>}
                </div>
              </div>
            );
          })()}
            ...
            <div className="card sales-card">
              <div className="row">
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ fontFamily: "var(--serif)", fontSize: 22, marginTop: 8, color: "var(--cream)" }}>{range === "all" ? ordersAll.length : range === "30d" ? orders30.length : orders7.length}</div>
                </div>
              </div>
              <svg className="spark" viewBox="0 0 600 64" preserveAspectRatio="none">
                ...
                <path d={sparkLine} fill="none" stroke="#2DD4BF" strokeWidth="2.5" />
                {sparkLast && <circle cx={sparkLast.x.toFixed(0)} cy={sparkLast.y.toFixed(0)} r="4" fill="#2DD4BF" />}
              </svg>
            </div>
          )}
        </div>
